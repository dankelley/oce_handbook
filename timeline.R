...
12.5,meet with Advisory Committee;|submit/defend Thesis Proposal;|submit Annual Progress Report
...
plot(c(-0.05,1), range(t), xlab="", ylab="", type="n", axes=FALSE)
box()
...
O <- par('usr')[1] + 0.05 * (par('usr')[2]-par('usr')[1])
arrows(rep(O, n), t, rep(par('usr')[1], n), t, length=0.07)
...
oce.axis.POSIXct(2, drawTimeRange=FALSE, tformat="%b")
#axis(2)
#abline(h=start+(1:3)*365*86400, col='gray')
if (multiline) {
    text(rep(0, n), t, sprintf("Month %d: %s", msc$month, gsub("\\|","\n",msc$action)), adj=0, cex=cex)
} else {
    text(rep(0, n), t, sprintf("Month %d: %s", msc$month, msc$action), adj=0, cex=cex)
