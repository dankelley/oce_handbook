...
\begin{itemize}
    ...
    \item ``Regular professor'' \fixme{fill in some text, with a link to FGS}

    \item ``Adjunct (x)'' \fixme{fill in some text, with a link to FGS}
...
%Report\furl{http://academiccalendar.dal.ca/Catalog/ViewCatalog.aspx?pageid=viewcatalog&catalogid=58&chapterid=2678&topicgroupid=10850}.\fixme{DK:
%I don't think we should state this ``along with.'' It makes it look as though
%submitting that document is sufficient. I would prefer to be vague on this; the
%GOC should probably take each case individually. This is not so common that we
%would be asking much of the GOC to take on this task.}\fixme{the links here are messed up}

\section{Supervision}

\fixme{the meeting on 2017-04-10 will address this section in the context of the FGS rules}\furl{http://academiccalendar.dal.ca/Catalog/ViewCatalog.aspx?pageid=viewcatalog&catalogid=58&chapterid=2678&topicgroupid=10852}



\provisional{Rules and guidelines for supervisors and {\supcom}s are provided in Section~9 of the \fgsregs. In addition,
the Department imposes the following requirements:
\begin{enumerate}
    \item The \supcom shall be formed within 1 month of the students entry into the academic program.

    \item Each \supcom shall have at least one member from outside the student's subdiscipline(s).

    \item Committees shall meet at least twice per year.

    \item Committee approval should be sought on significant milestones in the
        program, \eg MSc-PhD transfer, thesis defence, \etc.

    \item Within a week of the meeting, students must upload a meeting report
        to \dalonline. They are also responsible for ensuring ensure that their
        supervisor and the \GC submit approval within a reasonable time.
...
year of enrolling in the program.
...
\p The proposal is developed in consultation with the \supervisor and \supcom.
The scope of the research should be such that the program can be completed
within one year.
...
\supcom, unlike in the PhD case, where they are public events.
...
Students wishing to transfer should provide a letter to their \supcom
stating the reason for the transfer, and justification that they are
qualified for the PhD program. A one-page explanation is sufficient. The
student's \supcom is required to meet, and the \supcom may recommend
advancement to the PhD program.  A letter from the \supervisor, noting the date
when the \supcom met and justifying the advancement of the student to the PhD
...
approval is granted, the students must fill in a \formGSPU\furl{https://www.dal.ca/content/dam/dalhousie/pdf/fgs/currentstudents/Graduate_Student_Program_Update_Form.pdf}
...
\p The \supcom must be formed within the first month of the program.

\p The PhD \supcom consists of at least four members. There must be two
full-time faculty members from the Oceanography Department (not Adjunct FGS) on
the \supcom. If a regular faculty member serving on the committee leaves the
...
office at least 10 working days prior to the oral defence\vote{DK to GOC: I
think we should have a form for this. OK? DK=yes, BB=yes}  The proposal
